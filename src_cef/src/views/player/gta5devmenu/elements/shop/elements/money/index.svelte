<script>
    ...
    import money3 from '../../../../assets/money/3.png'
    import money4 from '../../../../assets/money/4.png'
    import money5 from '../../../../assets/money/5.png'
    ...
    const packs = [
        ...
        { rb: 10000, img: money4 },
        { rb: 40000, img: money5 },
        { rb: 200000, img: money6 },
    ];

    // Тот же курс, что и на сервере: amount * Convert.ToInt32(10 * DonateSettings.Convert)
    $: rate = Math.round(10 * $serverDonateDoubleConvert);

    const onChange = (pack) => {
        if ($accountRedbucks < pack.rb)
            return window.notificationAdd(4, 9, `Недостаточно Redbucks!`, 3000);
        executeClient ("client.donate.change", pack.rb);
        window.notificationAdd(2, 9, `Вы обменяли ${format("money", pack.rb)} RB на ${format("money", pack.rb * rate)}$`, 3000);
    }
</script>
...
<div class="list1">
    ...
    <div class="blockmoney">
        ...
        <div class="infoitem">
            <h1>{format("money", pack.rb * rate)}$</h1>
            <span>{format("money", pack.rb * rate)}$ на ваш игровой счёт</span>
            <div class="buyinfoitem">
                <p>
                    ...
                    {format("money", pack.rb)}
                </p>
                <div class="btnbuyitem" on:keypress on:click={() => onChange(pack)}>Приобрести</div>
            </div>
        </div>
    </div>
    {/each}
