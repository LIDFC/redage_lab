...
`conf.json`: включены `voice-chat` (без него не работает голосовой чат мода), выключены `allow-cef-debugging` и `enable-http-security` (с ней лаунчер не может скачать ресурсы до запуска игры — DLC-машины и интерьеры не подключаются). Чтобы сервер появился в списке RAGE:MP, поставьте `"announce": true` и задайте `name`.
