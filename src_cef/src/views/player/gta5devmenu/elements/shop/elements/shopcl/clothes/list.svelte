<script>
    ...
    import { clothesList, getRarity } from '../catalog.js'
    ...
    const sorters = {
        10: (a, b) => b.price - a.price,
        11: (a, b) => a.price - b.price,
        12: (a, b) => getRarity(b.price) - getRarity(a.price) || b.price - a.price,
        13: (a, b) => getRarity(a.price) - getRarity(b.price) || a.price - b.price,
    };

    $: isMale = !$charData || $charData.Gender !== false;
    $: search = (searchText || "").toLowerCase().trim();
    $: items = clothesList
        .filter((item) => item.type === category.type && item.male === isMale)
        .filter((item) => !search.length || item.name.toLowerCase().includes(search))
        .sort(sorters[pagefilter] || sorters[10]);

    let selectedId = -1;
    $: currentIndex = items.findIndex((item) => item.id === selectedId);
    $: selected = currentIndex > -1 ? items[currentIndex] : null;
    $: others = items.filter((item) => item.id !== selectedId).slice(0, 4);
    // Вернулись в «Все» — закрываем карточку товара
    $: if (pageload === 0) selectedId = -1;
    ...
    const onBuy = (item) => {
        ...
        // Результат покупки присылает сервер (успех / нет места в инвентаре)
...
{#if selected}
    <div class="selectblockcl">
        <div class="blgf act">
            <div class="headsbcl">
                <div class="shopback" on:keypress on:click={() => selectedId = -1}>&larr; Назад к списку</div>
                <h1>{selected.name}</h1>
                ...
                <div class="btnbuy" on:keypress on:click={() => onBuy (selected)}>Купить предмет</div>
            </div>
...
{:else if items.length || pageload === category.page}
    ...
    {#if !items.length}
        <p class="shopempty">{search.length ? "Ничего не найдено" : "В этом разделе пока нет товаров"}</p>
    {/if}
    <div class="list1">
